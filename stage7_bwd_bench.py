...
both Config B (BLKQ=64) and Config C (BLKQ=128) from
SLA/trition-autotune-benchmark-result.md.

Config B uses the Triton fwd + CK bwd cross-path introduced in Option 1.
Config C uses the full CK fwd + CK bwd path from Tier 2.5 Step 1.
"""
import os
import sys
import statistics
...
# MI355X published Triton autotune reference (from
# SLA/trition-autotune-benchmark-result.md):
...
def bench(attn, label, use_ck: bool, iters: int = 30, warmup: int = 5):
    ...
    # Separate fwd and bwd timing via cuda events (the stage7_bwd_bench
    # original was perf_counter-based around the whole fwd+bwd pair; keep
    # that too and additionally report fwd-only and bwd-only medians so
    # we can isolate the cross-path bwd win).
    totals = []
    fwds = []
    bwds = []
...
for name, BLKQ, BLKK in CONFIGS:
    pub_fwd, pub_bwd = PUBLISHED[name]
    print(f"\n=== Config {name}  "
          f"B={B} H={H} S={S} D={D} BLKQ={BLKQ} BLKK={BLKK} topk={TOPK} "
          f"(published Triton autotune: fwd {pub_fwd} | bwd {pub_bwd}) ===",
          flush=True)
    ...
    print(
        ...
        f"(vs published {pub_bwd}: {pub_bwd / ck_bwd:.3f}x)",
        flush=True,
    )
    print(
